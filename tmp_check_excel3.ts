...
async function main() {
  ...
  // Check sheets that had customer data: 한화 (플렉시볼), 한화, 하이크, 달러, 유로
  const sheetsToCheck = ['한화 (플렉시볼)', '한화', '하이크', '달러', '유로'];
  ...
  for (const sheetName of sheetsToCheck) {
    ...
    // Print ALL rows 1-15, ALL columns
    for (let r = 0; r <= Math.min(14, range.e.r); r++) {
      ...
      for (let c = 0; c <= range.e.c; c++) {
        ...
        if (cell && cell.v !== undefined && cell.v !== '') {
          rowData.push(`${XLSX.utils.encode_col(c)}${r+1}="${String(cell.v).substring(0, 50)}"`);
        }
      }
      if (rowData.length > 0) {
