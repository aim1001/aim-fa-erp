...
function DemoReportDialog({
  ...
  }, [open, assignedStaff, inquiry]);

  const handleDownload = async () => {
    if (!staffForm.name || !customerForm.company) {
      toast({ title: "작성자 이름과 고객사명은 필수입니다", variant: "destructive" });
      return;
    }
    setIsDownloading(true);
    try {
      const res = await fetch(`/api/inquiries/${inquiry.id}/demo-report/pdf`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({ staff: staffForm, customer: customerForm }),
      });
      if (!res.ok) {
      ...
      a.href = url;
      a.download = `test_report_${inquiry.inquiryNumber.replace(/[/\\:*?"<>|]/g, "_")}.pdf`;
      a.click();
      URL.revokeObjectURL(url);
      toast({ title: "테스트 리포트 PDF 다운로드 완료" });
      onOpenChange(false);
    } catch (err: any) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg" data-testid="dialog-demo-report">
        <DialogHeader>
          <DialogTitle data-testid="text-demo-report-title">데모 테스트 리포트</DialogTitle>
          <DialogDescription>작성자 및 고객 정보를 확인 후 PDF를 다운로드하세요.</DialogDescription>
        </DialogHeader>
            ...
            <div className="grid grid-cols-1 gap-2">
              <div>
                ...
                <Input
                  ...
                  placeholder="담당자 이름"
                  className="h-8 text-sm"
                  data-testid="input-demo-staff-name"
                />
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <Input
                    ...
                    className="h-8 text-sm"
                    data-testid="input-demo-staff-phone"
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    className="h-8 text-sm"
                    data-testid="input-demo-staff-email"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 gap-2">
              <div>
                ...
                <Input
                  ...
                  placeholder="고객사명"
                  className="h-8 text-sm"
                  data-testid="input-demo-customer-company"
                />
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <Input
                    ...
                    placeholder="담당자 이름"
                    className="h-8 text-sm"
                    data-testid="input-demo-customer-name"
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    onChange={e => setCustomerForm(f => ({ ...f, title: e.target.value }))}
                    placeholder="직함"
                    className="h-8 text-sm"
                    data-testid="input-demo-customer-title"
                  />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-2">
                ...
                <div>
                  ...
                  <Input
                    ...
                    placeholder="email@company.com"
                    className="h-8 text-sm"
                    data-testid="input-demo-customer-email"
                  />
                </div>
              </div>
            </div>
        ...
        <DialogFooter>
          <Button variant="outline" size="sm" onClick={() => onOpenChange(false)} data-testid="button-cancel-demo-report">취소</Button>
          <Button
            size="sm"
            onClick={handleDownload}
            disabled={isDownloading || !staffForm.name || !customerForm.company}
            data-testid="button-download-demo-report"
          >
            {isDownloading ? <Loader2 className="h-4 w-4 mr-1 animate-spin" /> : <FileDown className="h-4 w-4 mr-1" />}
